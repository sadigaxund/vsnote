...
| `Stepper` segmented-progress variant — R5-3 added a `doneLabel?: string` prop (default `"All steps done"`) and a sanctioned terminal state (`current >= steps.length` renders every segment done with the summary reading `Step {steps.length} of {steps.length} · {doneLabel}`, replacing the old undocumented "pass one index past the end" trick that produced a broken "Step 4 of 3" with a blank label) | `local/Stepper.tsx` (Publish dialog, Git & Sync) | [#35](https://github.com/sadigaxund/my-you-eye/issues/35) | open — comment posted describing this variant, local component in use meanwhile |
...
| `SettingsRow` / `SettingsSection` — R5-3 fixed a `controlWidth="full"` bug where a long-enough control column dropped onto its own line UNDER the label instead of staying beside it: the control column now sets `flex-basis: 0` (`flex: "1 1 0%"`) alongside `min-width: 0`, since a wrapping flex row decides line placement from an item's max-content size when its flex-basis is `auto` — `min-width: 0` alone only bounds shrinkage once already placed on a line, it doesn't affect that placement decision | every `settings/<Category>.tsx` module | [#34](https://github.com/sadigaxund/my-you-eye/issues/34) | open — local component in use meanwhile |
