...
import { Check, Copy, FileCode, Globe2, Loader2, Lock } from "lucide-react";
...
export function PublishDialog({
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (editMode && existingShare) {
        ...
        const updated = await updateShare(backendBaseUrl, existingShare.id, patch);
        setResult(updated);
      } else {
  ...
  const link = result ? buildShareLink(result, backendBaseUrl) : null;
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent size="md" data-testid="publish-dialog">
        <DialogHeader>
          <DialogTitle>{editMode ? "Edit share" : "Publish"}</DialogTitle>
          <DialogDescription>
            {editMode ? existingShare?.source_path : filePath ? `Share "${filePath.split("/").pop()}" with a link.` : ""}
          </DialogDescription>
        </DialogHeader>
        ...
        {!offline && authenticated && !result && (
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            <FormField label="Mode" hint={canRenderShare(fileKind) ? undefined : "Rendered mode is only available for Markdown and HTML files."}>
              <SegmentedControl
                size="sm"
                value={renderMode}
                ...
                options={RENDER_MODE_OPTIONS.map((o) => ({
                  ...o,
                  disabled: o.value === "rendered" && !canRenderShare(fileKind),
                }))}
              />
            </FormField>
          ...
          {result ? (
            ...
            <>
              ...
              <Button type="button" disabled={!canSubmit} onClick={handleSubmit} data-testid="publish-submit">
                {submitting ? <Loader2 size={13} className="animate-spin" /> : editMode ? "Save" : "Publish"}
              </Button>
            </>
          )}
        </DialogFooter>
