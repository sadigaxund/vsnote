 ...
 * origin now, so every share link — raw or rendered — points at THIS app's
 * own origin, `/share/{slug}`. There's no more "backend origin" distinct
 * from "app origin" to pick between: a real browser navigation to that URL
 * is content-negotiated server-side (`server/app/routers/share_public.py`'s
 * `_wants_html`) into either raw `text/plain`/binary (raw-mode shares —
 * never touches the SPA at all) or the SPA shell (rendered-mode shares,
 * which then fetches the real content itself via `Accept: application/json`)
 * — see that module's doc for the exact rule. This function no longer needs
 * to know `render_mode` at all to build the correct link.
 */
